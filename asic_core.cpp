...
void asic_core::push_gcn_cache_miss(pref_tuple cur_tuple) {
  if(_asic->_config->_update_coalesce) {
    ...
    } else { // here it should do compute-coalescing (push into cgra_event_queue)
...
void asic_core::insert_prefetch_process(int lane_id, int priority, pref_tuple cur_tuple) {
  if(_asic->_config->_algo!=gcn) priority=0;
...
void asic_core::create_async_task(DTYPE priority_order, task_entry new_task) {
  ...
  if(slice_num!=_asic->_current_slice && !_asic->_switched_cache) {
    if(_asic->_config->_algo==pr) { // not sure why?
      _asic->_stats->_stat_tot_num_copy_updates++;
    }
  } else { // get stats for the percentage used in the current slice..
  ...
  } else { // push into the task queue here...
...
int asic_core::access_all_edges(int line_addr, task_entry cur_task, pref_tuple next_tuple) {
  ...
  if(num_cache_lines_per_src<=edges_served) { // cool, good to go!
    edges_served = _asic->_offset[cur_task.vid+1]-cur_task.start_offset;
  } else { // serve only the edges that are left..
  ...
  if((_asic->_config->_exec_model==sync || _asic->_config->_exec_model==sync_slicing || _asic->_switched_sync) && cur_task.start_offset==_asic->_offset[cur_task.vid]) {
    ...
    if(cb_push<0) cb_push += COMPLETION_BUFFER_SIZE;
...
void asic_core::send_scalar_packet(pref_tuple cur_tuple) {
 ...
 int max_edges = _asic->_offset[cur_tuple.src_id+1]-_asic->_offset[cur_tuple.src_id];
  ...
  for(int i=0; i<core_cnt; ++i) {
    if(!_dest_done.test(i)) continue;
    // if(net_packet[i].src_id==-1) continue;
    // cout << "i: " << i << " dst_core: " << net_packet[i].dest_core_id << endl;
    // assert(net_packet[i].dest_core_id==i);

    if(i==src_core || _asic->_config->_perfect_net) { // push to the current core for all required entries
    ...
    } else { // push to the network which does the same thing
    ...
    // num_pops++;
        for(unsigned j=0; j<multicast_net_packet[i].multicast_dst_wgt.size(); ++j) {
          ...
          for(int l=0; l<num_packets; ++l) {
            ...
            for(int f=0; f<FEAT_LEN; ++f) return_to_arob.vertex_data[f] = cur_tuple.vertex_data[f];
#if GRAPHMAT_SLICING==1 && REUSE>1
             if(_asic->_finishing_phase) { // this should actually be access to the cache
               _asic->_asic_cores[dest_core]->insert_prefetch_process(0, 0, return_to_arob);
             } else {
...
void asic_core::send_multicast_packet(pref_tuple cur_tuple, int start_offset, int end_offset) {
   ...
   for(int i=start_offset; i<end_offset; ++i) {
     ...
     if(slice_num!=_asic->_current_slice) {
       ...
       if(l2hit) _asic->_mem_ctrl->_l2hits++;
     }
     ...
     if(src_core==dst_core) _asic->_debug_local++;
     else _asic->_debug_remote++;
...
void asic_core::consume_lsq_responses(int lane_id) {
  ...
  if((_asic->_config->_net!=crossbar && _asic->_config->_net==hrc_xbar) &&  _asic->_cur_cycle%_asic->_xbar_bw==0) return;
// TODO: Different kind of responses, based on how it was consumed
  ...
  while(!_pref_lsq[lane_id].empty() && can_push_in_process(lane_id)) {
    ...
    if(_asic->_config->_pr==0 && _asic->_config->_domain==graphs) { // could be src_dist for synchronous execution
...
} else { // Step 2: do something different for vectorized values? (no computation, directly send reduce packet over the network)
  ...
  while(!_pref_lsq[lane_id].empty() && num_pops<max_pops && _asic->_network->can_push_in_network(_core_id)) {
      ...
      if (_asic->_mem_ctrl->_num_pending_cache_reqs >= MAX_CACHE_MISS_ALLOWED) return;
#endif
      ...
      if(_asic->_config->_working_cache && !_asic->_config->_cache_hit_aware_sched) { // this is perfect net
      ...
      for(s=cur_tuple.start_edge_served; s<cur_tuple.end_edge_served; ++s) {
        ...
        assert(_asic->_asic_cores[dst_core]->_fine_grain_throughput>0 && "should push only to active cores");
         ...
         if(_asic->_asic_cores[dst_core]->can_push_in_process(0)) { // } || _asic->_reconfig_flushing_phase) {
           // _asic->_asic_cores[dst_core]->_prefetch_process[0].push_back(cur_tuple);
           for(int f=0; f<FEAT_LEN/16; ++f) { // this is like sending the source feature vector
           ...
           for(int x=(dst_core+1)%core_cnt, i=0; i<core_cnt; x=(x+1)%core_cnt, ++i) {
             ...
             if(cond && _asic->_asic_cores[x]->can_push_in_process(0)) { // } && cur_pref_size<min_pref_size) {
           ...
           if(_asic->_asic_cores[dst_core]->can_push_in_process(0)) { // } || _asic->_reconfig_flushing_phase) {
             ...
             if(_asic->_reconfig_flushing_phase) { // this must have been wasting a lot of cycles..in the initial phase
...
void asic_core::drain_aggregation_buffer(int index) {
  ...
  if(it==_aggregation_buffer[index].end()) return;
  _asic->_stats->_stat_online_tasks++;
...
void asic_core::spec_serve_atomic_requests() { // int lane_id) {
  ...
  for(int i=0; i<_asic->_config->_num_banks; ++i){
    ...
    if(!_asic->_bank_queues[i].empty()){
     ...
     if(cur_tuple.req_core_id!=-1) { // read request
     ...
     if(_asic->_config->_domain==tree) { // should always create new task for a tree
       found=false;
     }
...
// CONFLICT SEARCH ACROSS ALL QUEUES (BROADCAST)
      ...
      for(int c=0; c<core_cnt; ++c) {
        ...
        if(it!=_asic->_asic_cores[c]->_conflict_queue.end()) { // found
      ...
      if(!found) {
        ...
        if(it==_commit_queue.end()) { // not found, new timestamp
          queue<commit_info> a; a.push(x);
        ...
        if(_asic->_config->_domain==tree && _asic->is_leaf_node(new_task.vid)) { // create new task for leaf nodes otherwise normal task
...
void asic_core::commit() { // int lane_id) {
#if CF==0
  bool found=false;
  ...
  int cur_lowest_ts=-1;
  for(int c=0; c<core_cnt; ++c) {
    if(_asic->_asic_cores[c]->_commit_queue.empty()) continue;
...
// Update in memory
  ...
  if(_asic->_scratch_ctrl->_is_visited[x.vid]) return; 
...
void asic_core::pull_scratch_read() {
  ...
  while(elem_done<_fine_grain_throughput && !_prefetch_scratch.empty() && cb_entries_avail && _asic->_network->can_push_in_network(_core_id)) { // FIXME: it should be according to required packets..
    ...
    if(!_asic->_config->is_vector()) {
      ...
      if(_asic->_config->_inter_task_reorder==0) {
        ...
        bool first=true;
        if(cur_tuple.src_id==_last_requested_src_id) { // same vid again?
          first=false;
        } else {
        ...
        if(_edges_read_sent==in_degree) _last_requested_src_id=-1;
      ...
      for(int i=0; i<num_packets; ++i) { // address -- assume same core
        cur_tuple.cb_entry = _asic->_fine_reorder_buf[_core_id]->allocate_cb_entry(1);
        if(i==num_packets-1) {
          cur_tuple.last = last;
        } else {
...
void asic_core::process(int lane_id) {
  red_tuple next_tuple; pref_tuple cur_tuple;
  int elem_done=0;
...
void asic_core::reduce(int lane_id) {
  ...
  if(_asic->_config->_net_traffic==real_multicast) {
    ...
    while(elem_done<_active_process_thr && !_process_reduce[lane_id].empty() && _asic->_network->can_push_in_network(_core_id) && _asic->can_push_to_bank_crossbar_buffer()) {
      if(_asic->_config->_working_cache) {
        if(_asic->_mem_ctrl->_num_pending_cache_reqs>=(MAX_CACHE_MISS_ALLOWED-num_banks)) break;
        if(_asic->_stats->_stat_pending_colaesces>=(MAX_COALESCES_ALLOWED-num_banks)) break;
      ...
      if(_asic->_config->_pull_mode) { 
        ...
        } else { // Diff1: reduce using scratch data, Diff2: if updated, insert new task
          if(_asic->_config->_entry_abort) {
            _asic->_task_ctrl->_present_in_local_queue[_core_id].reset(cur_tuple.src_id);
          }
          if(_asic->_config->_algo==pr) {
      ...
      if(_asic->_config->_net==crossbar) bank_id = _asic->_scratch_ctrl->get_global_bank_id(cur_tuple.dst_id);
      ...
      if(send_over_net) {
        ...
        if(_asic->_config->_net==mesh) {
          if(_asic->_config->_net_traffic==real_multicast) {
            ...
            if(new_packet) { // send the collected ones
  ...
  assert(!_task_queue[tqid][lane_id].empty() || _pending_task[tqid].vid!=-1);
   ...
   while(num_task_queue_pops<max_task_queue_pops && local_elem_disp < max_local_elem_dispatch && _asic->_elem_dispatch<_asic->_max_elem_dispatch_per_core && (!_task_queue[tqid][lane_id].empty() || _pending_task[tqid].vid!=-1) && num_pops<_fine_grain_throughput && num_cache_access<_max_cache_bw && _asic->_compl_buf[_core_id]->_entries_remaining>2) { // TODO: check for dispatch condition here as well!!
    ...
    /*if(!_asic->_config->no_swarm()) {
     ...
     int min_cb=1;
     int edges_served=0;
     if(_asic->_config->is_sync()) min_cb=2;
     assert(cur_task.start_offset<=_asic->_offset[src_id+1] && "cannot start afte end of edge list");
     for(i=cur_task.start_offset; _asic->_elem_dispatch<_asic->_max_elem_dispatch_per_core && local_elem_disp < max_local_elem_dispatch && i<_asic->_offset[src_id+1] 
     && num_pops<_fine_grain_throughput && num_cache_access<_max_cache_bw && !_swarm_pending_request[lane_id] && _in_order_flag[lane_id] && _asic->_compl_buf[_core_id]->_entries_remaining>min_cb; ++i) {
       ...
       ++total_edges_requested;
        ...
        if(_asic->_config->_dramsim_enabled==1) {
          ...
          if(_asic->_config->_all_edge_access==1) { // FIXME: very weirdly I access vectorized edges here
            // cur_task.start_offset = _asic->_offset[cur_task.vid+1];
            i = cur_task.start_offset;
            break;
          }
      ...
      */
      ...
      if(_asic->_config->_numa_contention) {
        ...
        if(pref_tuple.dest_core_id!=_core_id) {
          ...
          for(int i=0; i<edges_served/(line_size/bus_width); ++i) { // LINK_BW
      ...
      if(i==_asic->_offset[src_id+1]) {
        ...
        /* if((_asic->_config->_algo==pr) && (_asic->_config->_exec_model==sync || _asic->_switched_sync)) {
          ...
          if(_pending_task[tqid].vid!=-1) {
            ...
            } else if(_asic->_task_ctrl->_remaining_task_entries[_core_id][tqid]<HIGH_PRIO_RESERVE && _fifo_task_queue.size()>0) { // just so as i don't break the order of fifo
            ...
            if(should_put_in_tq) { // popping from task queue here...
        ...
        } else { // re-push with updated priority
          (it->second).pop_front();
          if((it->second).empty()) {
            _task_queue[tqid][lane_id].erase(it->first);
          }
...
bool asic_core::local_pipeline_inactive(bool show) {
  if(!_asic->_fine_reorder_buf[_core_id]->pipeline_inactive(show)) {
    if(show) cout << "Local fine reorder buf is not empty: " << _asic->_fine_reorder_buf[_core_id]->_entries_remaining << endl;
    return false;
  }
  if(_asic->_stats->_stat_pending_cache_misses!=0) {
    if(show) cout << "Pending cache misses\n";
    return false;
  }
  // check if cgra event queue is empty
  for(int d=0; d<MAX_DFGS; ++d) {
    if(!_cgra_event_queue[d].empty()) {
      if(show) cout << "CGRA event queue is not empty: " << _cgra_event_queue[d].size() << "\n";
      return false;
    }
  }

  if(!_priority_hit_gcn_updates.empty()) {
    if(show) cout << "Priority hit GCN update queue is not empty: " << _priority_hit_gcn_updates.size() << "\n";
    return false;
  }
  if(!_hit_gcn_updates.empty()) {
    if(show) cout << "Hit GCN update queue is not empty: " << _hit_gcn_updates.size() << "\n";
    return false;
  }
  if(!_miss_gcn_updates.empty()) {
    if(show) cout << "Miss GCN update queue is not empty: " << _miss_gcn_updates.size() << "\n";
    return false;
  }
  if(!_coarse_cgra_event_queue.empty()) {
    if(show) cout << "Multiply CGRA event queue is not empty: " << _coarse_cgra_event_queue.size() << "\n";
    return false;
  }
  ...
  if(!_hol_conflict_queue.empty()) {
    if(show) cout << "Conflict queue for gcn is not empty" << endl;
    return false;
  }
  for(int i=0; i<_asic->_config->_lane_width; ++i) {
    for(int j=0; j<NUM_TQ_PER_CORE; ++j) {
      if(!_task_queue[j][i].empty()) { 
        if(show) cout << "Local task queue not empty with size: " << _task_queue[j][i].size() << " at lane id: " << i << " tq id: " << j << " and inorder flag: " << _in_order_flag[i] << endl; 
        return false;
      }
    }
    if(!_prefetch_process[i].empty()) { 
      if(show) cout << "prefetch_process queue not empty: " << _prefetch_process[i].size() << endl; 
      return false;
    }
    if(!_pref_lsq[i].empty()) { 
      if(show) cout << "prefetch_lsq queue not empty: " << _pref_lsq[i].size() << endl; 
      return false;
    }
    if(!_process_reduce[i].empty()) { 
      if(show) cout << "_process_reduce queue not empty: " << _process_reduce[i].size() << endl; 
      return false;
    }
  }
  if(!_prefetch_scratch.empty()) {
    if(show) cout << "fine prefetch_scratch queue not empty: " << _prefetch_scratch.size() << endl; 
    return false;
  }
  if(!_scratch_process.empty()) {
    if(show) cout << "fine scratch_process queue not empty: " << _scratch_process.size() << endl; 
    return false;
  }
  ...
  if(_asic->_config->_prac==1) {
    for(int i=0; i<NUM_TQ_PER_CORE; ++i) {
      if(!_aggregation_buffer[i].empty()) { 
        if(show) cout << "aggregation buffer queue not empty: " << _aggregation_buffer[i].size() << endl; 
        return false;
      }
    }
  }
  ...
  for(int i=0; i<num_banks; ++i) {
    if(_asic->_config->_net==crossbar) {
      if(!_asic->_bank_queues[i].empty()) {
        if(show) cout << "global bank queues not empty with index: " << i << " and size: " << _asic->_bank_queues[i].size() << "\n";
        return false;
      }
    } else {
      if(!_local_bank_queues[i].empty()) {
        if(show) cout << "local bank queues not empty with index: " << i << " and size: " << _local_bank_queues[i].size() << "\n";
        return false;
      }
    }
  }
  if(!_fifo_task_queue.empty()) {
    if(show) cout << "FIFO task queues not empty\n";
    return false;
  }
  return true;
}
...
bool asic_core::pipeline_inactive(bool show) {
  if(!_asic->_task_ctrl->_pending_updates.empty()) {
    if(show) cout << "updates are pending in an overflow buffer: " << _asic->_task_ctrl->_pending_updates.size() << endl;
    return false;
  }
  if(!_asic->_fine_reorder_buf[_core_id]->pipeline_inactive(show)) {
    if(show) cout << "Local fine reorder buf is not empty: " << _asic->_fine_reorder_buf[_core_id]->_entries_remaining << endl;
  ...
  if(_asic->_delayed_net_tasks!=0) {
    if(show) cout << "Tasks in the network delay buffer\n";
    return false;
  }
  if(_asic->_mem_ctrl->_cur_writes!=0) {
    if(show) cout << "Pending scatter writes in SGU slicing\n";
    return false;
  }
  if(_asic->_stats->_stat_pending_cache_misses!=0) {
    if(show) cout << "Pending cache misses\n";
  ...
  if(_asic->_config->_sgu_gcn_reorder==0 && _cur_arob_push_ptr!=_cur_arob_pop_ptr) {
    if(show) cout << "Atomic update ROB not empty with push ptr: " << _cur_arob_push_ptr << " and pop ptr: " << _cur_arob_pop_ptr << endl; 
    return false;
  }
  if(!_asic->_task_ctrl->_global_task_queue.empty()) { 
    if(show) cout << "Global task queue not empty with size: " << _asic->_task_ctrl->_global_task_queue.size() << endl; 
    return false;
  }
  if(!_crossbar_input.empty()) { 
    if(show) cout << "Crossbar input for graphicionado not empty with size: " << _crossbar_input.size() << endl; 
    return false;
  }
  ...
  for(int i=0; i<_asic->_config->_lane_width; ++i) {
    ...
    if(!_pending_swarm[i].empty()) { 
      if(show) cout << "pending swarm queue not empty: " << _prefetch_process[i].size() << endl; 
      return false;
    }
    if(!_prefetch_process[i].empty()) { 
      if(show) cout << "prefetch_process queue not empty: " << _prefetch_process[i].size() << endl; 
...
// NEW QUEUES FOR SGU ONLY
  if(_asic->_config->_exec_model==async) {
    if(!_stall_buffer.empty()) { 
      if(show) cout << "stall buffer queue not empty: " << _stall_buffer.size() << endl; 
      return false;
    }
  }
  if(_asic->_config->_prac==1) {
  ...
  if(_commit_queue.size()!=0) {
    if(show) cout << "Commit queue size not empty\n";
    return false;
  }
  if(_asic->_mem_ctrl->_outstanding_mem_req.size()!=0) {
    if(show) cout << "Outstanding mem req size not empty\n";
    return false;
  }
...
bool graphmat::cycle() {
  ...
  if(_asic->_config->is_vector()) {
     ...
     for(int i=0; i<_fine_grain_throughput/process_thr; ++i) { // this is 1
       for(int j=0; j<_asic->_config->_lane_width; ++j) { // this is 1 for accel
         forward_atomic_updates(i, j);
         forward_multiply_opn(j);
       }
       if(1) { // _asic->_cur_cycle%x==0) {
  ...
  if(_asic->_config->_prac==1 && _asic->_switched_async) {
    if(_asic->_cur_cycle%2) { // this should be drained every 2 cycles...
      for(int j=0; j<NUM_TQ_PER_CORE; ++j) {
...
void dijkstra::commit() { // int lane_id) {
...
bool dijkstra::cycle() {
  ...
  if(is_done) { // if all visited, copy to memory and send done
...
bool sgu::cycle() {
  ...
  if(_asic->_config->_prac==1 && _asic->_cur_cycle%2) { // this should be drained every 2 cycles...
    for(int j=0; j<NUM_TQ_PER_CORE; ++j) {
      drain_aggregation_buffer(j);
    }
  }
  ...
  if(is_done && !_asic->_switched_sync && !_asic->_config->is_vector()) { // if all visited, copy to memory and send done
    for(int i=0; i<_asic->_graph_vertices; ++i) {
      _asic->_vertex_data[i] = _asic->_scratch[i];
...
#endif
  if(_asic->_config->_pull_mode) return false;
  // return false;
...
// queue<pref_tuple>* asic_core::select_queue_for_cache_hit() {
  bool asic_core::select_queue_for_cache_hit() {
    ...
    if(_slice_execution) { // just serve hits
...
bool asic_core::is_queue_head_hit(deque<pref_tuple> *target_queue){
    if(target_queue->empty()) return false;
    pref_tuple cur_tuple = target_queue->front();
    uint64_t paddr = _asic->_mem_ctrl->_vertex_offset + (cur_tuple.edge.dst_id * message_size * FEAT_LEN);
    bool l2hit = _asic->_mem_ctrl->is_cache_hit(_core_id, paddr, ACCESS_LOAD, true, 0);
    if(l2hit) return true;
    else {
      if(l2hit) return true;
      else {
        if(_asic->_task_ctrl->_present_in_miss_gcn_queue[cur_tuple.edge.dst_id]==0) {
...
void asic_core::cf_reduce(int lane_id, int dfg_id) {
  ...
  if (_asic->_config->_net != crossbar && _asic->_config->_net == hrc_xbar) {
    if (_asic->_cur_cycle % _asic->_xbar_bw == 0) return;
  }
  ...
  if (_asic->_config->_cache_hit_aware_sched && _asic->_config->_working_cache==1) {
    if (!_priority_hit_gcn_updates.empty()) {
      ...
      is_prefetch = false;
      if (!_slice_execution) { // when overlap doesn't cause thrashing
        can_serve_miss_updates();
      }
      ...
      if(_prefetch_process[0].empty()) return;
  ...
  } else if(_asic->_config->_working_cache==1) { // TODO: should it sent miss request in parallel? I guess Yes!!
    if (!_priority_hit_gcn_updates.empty()) {
      target_queue = &_priority_hit_gcn_updates;
      is_prefetch = false;
      ++_asic->_stats->_stat_tot_priority_hit_updates_served[_core_id];
  ...
  if(_asic->_config->_algo==gcn) {
    if(_asic->_stats->_stat_barrier_count==0) feat_len = FEAT_LEN1;
    if(_asic->_stats->_stat_barrier_count==1) feat_len = FEAT_LEN2;
  }
  ...
  if(!_asic->_config->no_swarm()) {
    if(!go_with_prefetch) lane_id=0; // cur_tuple.lane_id;
  } else {
    assert(lane_id==0);
  }
  ...
  if((!go_with_prefetch || !(*target_queue).empty() || !_prefetch_process[0].empty()) && can_push_to_cgra_event_queue()) {
    ...
    if(!go_with_prefetch) assert(!conflict);
    ...
    if(!conflict) {
      ...
      if(_asic->_config->_algo==cf) { // TODO: for the pull mode, this is done in a loop for the incoming edges; and the condition is same for spawn
        ...
        if(_asic->_config->_pull_mode || cur_tuple.end_edge_served==all_edges_received) { // last works for pull mode
          if(_asic->_config->_pull_mode) { // FIX THIS...just identify this end somehow; and we should not need dst_id and wgt from the tuple...
            if(cur_tuple.last) { // correct only twice, do not need that check
              if(_asic->_config->_entry_abort) {
                _asic->_task_ctrl->_present_in_local_queue[_core_id].reset(cur_tuple.src_id);
              }
...
void asic_core::issue_requests_from_pending_buffer(int lane_id) {
  ...
  if(_swarm_pending_request[lane_id]) return;
  if(_pending_swarm[lane_id].empty()) return;
...
void asic_core::automatic_partition_profile() {
  ...
  while(tasks_issued<max_tasks_per_cycle && edges_served<max_edges_per_cycle && !_task_queue[0][0].empty()) {
    ...
    if(_asic->_map_vertex_to_core[vid]==-1) { // Step1: never mapped
      ...
      if(_asic->_stats->_stat_dist_tasks_dequed[_core_id]<_asic->_scratch_ctrl->_slice_size) {
        _asic->_stats->_stat_dist_tasks_dequed[_core_id]++;
      } else { // TODO: need to evict some other data (ie. change its mapping)
     ...
     if(cur_mapped_core!=_core_id) {
       ...
       if(_asic->_inc_freq[_core_id][vid] > _asic->_inc_freq[cur_mapped_core][vid]) {
         ...
         if(_asic->_stats->_stat_dist_tasks_dequed[_core_id]<_asic->_scratch_ctrl->_slice_size) {
           _asic->_stats->_stat_dist_tasks_dequed[_core_id]++;
         } else { // TODO: need to evict some other data
           int evict_vid = choose_eviction_cand();
           int evict_core = get_evict_core(evict_vid);
  ...
  // if(0) { // task queue is empty, then work steal?
    if(edges_served==0) { // task queue is empty, then work steal?
    ...
    if(steal_core==-1 || _core_id==steal_core) return;
...
void asic_core::delete_one_timestamp(int dst_id, int tid, int dst_timestamp) {
 ...
 // set the current entry to invalid in dependence queue
  ...
  for(auto it=_task_queue[0][0][dst_timestamp].begin(); it!= _task_queue[0][0][dst_timestamp].end();) {
    ...
    if(cur_task.vid==dst_id && cur_task.tid==tid) { // to assure we are deleting the right one
      found=true;
