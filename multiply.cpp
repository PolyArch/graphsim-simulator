...
int multiply::dispatch(int dfg_id, vector<int> rows, vector<int> weight_rows_done, vector<bool> second_buffer) { // prefetch
  /*if(_core_id==0) {
  ...
  */
      ...
      for(int i=0; i<num_cache_lines_per_leaf; ++i) {
        ...
        if(hit) {
          ...
          if((!_asic->_config->_update_coalesce) || (_asic->_config->_update_coalesce && _asic->_task_ctrl->_present_in_miss_gcn_queue[rows[0]]==0)) {
            // cout << "it was miss\n";
            mult_task same_task; same_task.row = rows[0];
  ...
  for(i=weight_rows_done[0]; i<FEAT_LEN && _asic->_coarse_compl_buf[_core_id]->can_push(); ++i) {
    ...
    for(int c=0; c<num_cache_lines_per_weight; ++c) {
      ...
      if(sent_request) { _num_wgt_reqs_really_sent++; }
    }
  }
  if(i!=FEAT_LEN) {
    // re-push this task
...
int multiply::serve_local_coalescer_requests() {
  if(_asic->_local_coalescer[_core_id]->_entries_in_priority_update==0) { // if empty..
    return 0;
  }
  float batch_size=0;
  ...
  ++_stalled;
  if(_stalled==hrst_stall) _stalled=-hrst_stall;
  if(((_asic->_cur_cycle-_phase_cycle)<LSQ_WAIT || (batch_size<_asic->_local_coalescer[_core_id]->_multicast_batch_size/2)) && _stalled>0 && _stalled<hrst_stall) { // there should be some delay somewhere; maybe for total batching available?
  ...
  if(temp.first==-1 && temp.second==-1) return 0;
...
int multiply::serve_local_hats_requests() {
  ...
  if(_asic->_remote_coalescer[_core_id]->_entries_in_priority_update==0) { // if empty..
    /*
  ...
  if(((_asic->_cur_cycle-_phase_cycle)<LSQ_WAIT || (batch_size<_asic->_remote_coalescer[_core_id]->_multicast_batch_size/2)) && _stalled>0 && _stalled<hrst_stall) { // there should be some delay somewhere; maybe for total batching available?
 ...
 // cout << "Issuing at cycle: " << _asic->_cur_cycle << " core: " << _core_id << " dfg_id: " << cur_tuple.dfg_id 
  ...
  if(_asic->_config->_domain==tree) { // send with temporal reuse
    _asic->_remote_coalescer[_core_id]->send_batched_compute_request(leaf_id, cur_tuple, index_into_batched);
  } else { // batch and reduce only a pack, send a multicast scratch packet that will push to scratch write
...
void multiply::push_scratch_data(int dfg_id, pref_tuple cur_tuple) { // consume the loaded values
  ...
  for(int c=0; c<1; ++c) { // number of cache lines... (why entry cycle: need better names)
    mult_data next_tuple;
...
int multiply::get_updated_insert_cycle(int dfg_id) {
   ...
   if(new_push_cycle==_last_push_cycle[dfg_id]) { // integer
     new_push_cycle += 1;
   } else { // do not add
...
void multiply::push_lsq_data(pref_tuple cur_tuple) { // consume the loaded values
...
void multiply::process(int dfg_id) {
  ...
  while(num_pops<1 && latency_cond && can_push_in_process_reduce(dfg_id)) { // only 1 pop
    ++num_pops;
    mult_data cur_tuple = _lsq_process[dfg_id].front();
    mult_data dot_product = cur_tuple;
    int dotp=0;
    cur_tuple.vec1.clear(); cur_tuple.vec2.clear();
...
void multiply::reduce(int dfg_id) {
  ...
  while(num_pops<1 && !_process_reduce[dfg_id].empty()) { // only 1 pop
    ...
    if(output.second_buffer) ++_asic->_odd_phase_matrix_mult;
    else ++_asic->_even_phase_matrix_mult;
    ...
    if(output.second_buffer && _asic->_gcn_even_phase==false) allowed_to_create=true;
    if(!output.second_buffer && _asic->_gcn_even_phase==true) allowed_to_create=true;
...
bool multiply::can_push_local_coarse_grain_task(int dfg_id) {
  ...
  if(!are_two_tasks_ready && !_local_coarse_task_queue.empty()) {
    if(_asic->_cur_cycle-_local_coarse_task_queue.front().entry_cycle>MAX_DELAY) return true;
  }
  return false;
}
...
bool multiply::cycle(int phase) {
  int served=0;
  if(_asic->_config->_hats==1) { //  && _asic->_config->_domain==tree) {
  ...
  for(int d=0; d<_data_parallel_throughput; ++d) {
    ...
    while(!_asic->_coarse_compl_buf[_core_id][d]->_pref_lsq.empty()) {
      if(can_push_in_process(d)) {
        ...
        for(int r=0; r<cur_tuple.repeat_times; ++r) { // should push to different cores
          if(_asic->_config->_update_coalesce && _asic->_config->_mult_cache_hit_aware_sched) {
            ...
            if(_asic->_config->_batched_cores>1) {
              ...
              if(last_core==end_core) last_core = start_core;
            } else {
              push_lsq_data(cur_tuple);
            }
          }
        }
      } else { // push to other core
       ...
       /*if(_core_id==14) {
        ...
        if(_asic->_config->_algo==gcn) {
          ...
          if(num_done==FEAT_LEN) {
            if(_pending_task[d].weight_rows_done>0) { // taken from pending buffer
            ...
            } else { // pushing to pending buffer
...
bool multiply::pipeline_inactive(bool show) {
  if(!_asic->_local_coalescer[_core_id]->pipeline_inactive(show)) {
    if(show) cout << "Local coalescer was active" << endl;
    return false;
  }
  if(!_asic->_remote_coalescer[_core_id]->pipeline_inactive(show)) {
    if(show) cout << "Remote coalescer was active" << endl;
    return false;
  }
  if(!_asic->_task_ctrl->_pending_coarse_buffer.empty()) {
      if(show) cout << "pending coarse buffer queue not empty: " << _asic->_task_ctrl->_pending_coarse_buffer.size() << endl;
      return false;
  }
  if(!_prefetch_lsq.empty()) { 
    if(show) cout << "prefetch_lsq queue not empty: " << _prefetch_lsq.size() << endl; 
    return false;
  }
  for(int i=0; i<MAX_DFGS; ++i) {
    if(_asic->_cur_cycle<_lane_blocked_until[i]) {
      if(show) cout  << "Cur lane blocked for temporal reuse at core: " << _core_id << " dfg: " << i << " until: " << _lane_blocked_until[i] << endl;
      return false;
    }
    if(!_asic->_coarse_compl_buf[_core_id][i]->pipeline_inactive(show)) {
      if(show) cout << "Local compl buf is not empty: " << _asic->_coarse_compl_buf[_core_id][i]->_entries_remaining << endl;
      return false;
    }
    if(!_lsq_process[i].empty()) { 
      if(show) cout << "lsq_process queue not empty: " << _lsq_process[i].size() << " at core: " << _core_id << " dfg_id: " << i << endl; 
    ...
    if(!_process_reduce[i].empty()) { 
      if(show) cout << "process_reduce queue not empty: " << _process_reduce[i].size() << endl; 
      return false;
    }
    if(!_scratch_process[i].empty()) {
      if(show) cout << "scratch_process queue not empty: " << _scratch_process[i].size() << endl;
      return false;
    }
    if(!_asic->_coarse_reorder_buf[_core_id][i]->pipeline_inactive(show)) {
      if(show) cout << "Local reorder buf is not empty: " << _asic->_coarse_reorder_buf[_core_id][i]->_entries_remaining << endl;
      return false;
    }
    if(_pending_task[i].weight_rows_done>0) {
      if(show) cout << "Buffer for pending task is not empty: " << _pending_task[i].weight_rows_done << endl;
      return false;
    }
  }

  if(!_asic->_task_ctrl->_coarse_task_queue.empty()) {
    if(show) cout << "Coarse grained task queue is not empty: " << _asic->_task_ctrl->_coarse_task_queue.size() << endl;
    return false;
  }
  if(!_local_coarse_task_queue.empty()) {
    if(show) cout << "Local coarse grained task queue is not empty: " << _local_coarse_task_queue.size() << " and core: " << _core_id << endl;
    return false;
  }
  if(!_local_miss_task_queue.empty()) {
    if(show) cout << "Local miss task queue is not empty: " << _local_miss_task_queue.size() << endl;
    return false;
  }
  if(_asic->_pending_hats_requests!=0) {
    if(show) cout << "Pending hats requests is not empty: " << _asic->_pending_hats_requests << endl;
    return false;
  }
...
int multiply::serve_hats_requests() {
  ...
  while(_asic->_pending_hats_requests>0 && _asic->_leaf_access_count[_asic->_prev_leaf_id]==0 && count < _asic->num_kdtree_leaves) {
    ...
    ++count;
    if(_asic->_prev_leaf_id==0) { // or if pending request is less than X, then we should stall.
      ++_asic->_round;
  ...
  ++_asic->_stats->_num_spatial_issue; _asic->_stats->_num_tasks_batched += to_reduce;
