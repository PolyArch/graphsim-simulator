...
void coalescing_buffer::send_batched_compute_request(int leaf_id, pref_tuple cur_tuple, int index_into_batched) {
  ...
  cur_tuple.req_core=_core_id; cur_tuple.core_type=coarseMem;
  cur_tuple.src_id = leaf_id; cur_tuple.second_buffer = false;
  ...
  for(int c=0; c<num_cache_lines_per_leaf; ++c) {
    ...
    if(sent_request || _asic->_mem_ctrl->_l2coalesces>old_coalesces) { // if coalesced or sent request
      all_hits=false;
    }
  ...
  if(all_hits) {
    int d = cur_tuple.dfg_id;
     int min_pref_size = _asic->_mult_cores[_core_id]->_mult_fifo_len;
       ...
       for(int c=0; c<num_cache_lines_per_leaf && new_dst_core>-1; ++c) { // repeat_times for each..
...
void coalescing_buffer::send_batched_read_response(int index_into_batched) {
 ...
 // to wait for too long. (I need to fix that although shouldn't be an error)
  ...
  net_tuple.multicast_dst_core_id.clear(); net_tuple.multicast_dst_wgt.clear();
    ...
    for(auto it2=(it->second).begin(); it2!=(it->second).end(); ++it2) {
      ...
      if(priority!=old_priority) {
        ...
        if(_priority_update_queue.find(priority)==_priority_update_queue.end()) {
          list<pair<int, int>> x; x.push_back(make_pair(leaf_id, it2->second));
...
bool coalescing_buffer::pipeline_inactive(bool show) {
  if(!_overflow_for_updates.empty()) {
    if(show) cout << "Overflow queue for updates at core: " << _core_id << " is not empty: " << _overflow_for_updates.size() << endl;
    return false;
  }
  if(!_priority_update_queue.empty()) {
    if(show) cout << "Priority update queue for core: " << _core_id << " is not empty: " << _priority_update_queue.size() << endl;
    return false;
  }
  return true;
}
...
void coalescing_buffer::insert_leaf_task(int leaf_id, int req_core, int cb_entry, int dfg_id) {
   ...
   if(cur_leaf_present) {
     for(auto it=_priority_update_queue.begin(); it!=_priority_update_queue.end(); ++it) {
       for(auto it2=(it->second).begin(); it2!=(it->second).end(); ++it2) {
         if(it2->first==leaf_id) { // should not be just leaf_id for a fixed length buffer
           ...
           if(cur_leaf_valid) break; // it will chose the one that is valid
         }
       }
     }
   }
   ...
   if(cur_leaf_present && cur_leaf_valid) { // valid for width, present for length
     ...
     if(_entries_in_priority_update<_batch_length && !_batch_free_list.empty()) { // push a new entry for both scenarios
       ...
       if(it==_priority_update_queue.end()) {
         list<pair<int, int>> x; x.push_back(make_pair(leaf_id, pointer_into_batch));
         _priority_update_queue.insert(make_pair(priority, x));
       } else {
       ...
       ++_num_batched_tasks_per_index[pointer_into_batch];
       _data_of_batched_tasks[_num_batched_tasks_per_index[pointer_into_batch]-1][pointer_into_batch] = data;
     } else { // push to the overflow buffer
       --_total_current_entries;
       if(_asic->_config->_domain==tree) { // if remote read, set the data to req_core
...
bool completion_buffer::pipeline_inactive(bool show) {
  if(_cur_cb_push_ptr!=_cur_cb_pop_ptr) {
    if(show) cout << "Pending memory reqs" << endl; 
    return false;
  }
  if(_entries_remaining!=COMPLETION_BUFFER_SIZE) {
    if(show) cout << "Completion buffer not empty" << endl; 
    return false;
  }
  if(!_pref_lsq.empty()) {
    if(show) cout << "Pref lsq not empty" << endl; 
    return false;
  }
...
void completion_buffer::cb_to_lsq() {
  ...
  for(int i=init_start_point; i<COMPLETION_BUFFER_SIZE; ++i) {
    if(!_reorder_buf[i].valid) return;
    if(!can_push_in_pref_lsq()) return;
    _entries_remaining++;
...
int completion_buffer::deallocate_cb_entry(int waiting_count) {
  ...
  if(cb_entry<0) cb_entry += COMPLETION_BUFFER_SIZE;
  assert(waiting_count>0);
  _reorder_buf[cb_entry].waiting_addr=0;
  _reorder_buf[cb_entry].valid=false;
  _entries_remaining++;
  _cur_cb_push_ptr = (_cur_cb_push_ptr-1)%COMPLETION_BUFFER_SIZE;
  if(_cur_cb_push_ptr<0) _cur_cb_push_ptr += COMPLETION_BUFFER_SIZE;
  return cb_entry;
}
...
void asic::call_simulate() {
  ...
  switch(_config->_exec_model) {
    case async_slicing: simulate_slice(); // simulate_sgu_slice();
                    break;
    case sync_slicing: simulate_slice(); // simulate_graphmat_slice();
                    break;
    // case blocked_async: simulate_slice(); // simulate_blocked_async_slice();
    case blocked_async: simulate_blocked_async_slice();
                    break;
    case dyn_graph: simulate_dyn_graph();
                    break;
    default: simulate();
             break;
  }
...
void asic::reset_algo_switching() {
  _cur_cycle=0;
  _global_iteration=0;
  _mem_ctrl->_l2hits=0; _mem_ctrl->_l2accesses=0;
...
void asic::assign_rand_tstamp() {
  ...
  for(int i=0; i<_dyn_tot_parts; ++i) edges_assigned[i]=0;
  for(int i=0; i<_dyn_tot_parts; ++i) _tot_map_ind[i]=i;
  ...
  for(int i=0; i<half_edge; ++i) {
    ...
    if(temp_partitions>1 && edges_assigned[temp_stamp]==_dyn_batch_size) { // iteration size...a new parameter (should be send while simulation
      temp_partitions--;
  ...
  for(int i=0; i<_dyn_tot_parts; ++i) _tot_map_ind[i]=0;
  for(int i=0; i<half_edge; ++i) {
...
void asic::rand_edge_update_stream(int part) {
  ...
  if(_graph_edges==half_edge) {
    for(int i=0; i<half_edge; ++i) {
      if(_original_neighbor[i].src_id!=prev_vid) { // Is this wrong?
    ...
    for(int i=prev_vid+1; i<=_graph_vertices; ++i) _offset[i]=_offset[i-1];
  ...
  for(int i=half_edge; i<corner; ++i) { // insert all previous timestamps
  ...
  for(int i=prev_vid+1; i<=_graph_vertices; ++i) _offset[i]=count;
...
void asic::simulate_dyn_graph() {
  ...
  for(int cur_edges=original_graph_edges/2, iter=0; iter<(_dyn_tot_parts+1) && cur_edges<original_graph_edges; cur_edges+=_dyn_batch_size, ++iter) { 
    ...
    if(vert_prev_comp!=0) { // we do not need to activate if it is the first case
    int impacted_vertices=0;
#if SEQ_EDGE==0
      // nothing...we already inserted there!
#else
      for(int v=vert_prev_comp; v<_cur_used_vert; ++v) {
        if(_vertex_data[v]==MAX_TIMESTAMP) continue;
...
asic::asic() {
  ...
  if(_config->is_async()) {
    if(_config->_pull_mode) { // 10 infinite simulation, 15 nan too late
  ...
  } else if(_config->_blocked_async) { // blocked async
    _gama *= 100;
  } else {
    _gama *= 1;
  }
...
#if ABCD==1
    for(int i=0; i<SLICE_COUNT; ++i) _grad[i]=0;
#endif
  ...
  for(int e=0; e<_graph_edges; ++e) _edge_freq[e]=0;
#endif
...
// #endif
...
#if LADIES_GCN==1
    for(int i = 0; i < LADIES_SAMPLE; i++)  _is_dest_visited.reset(i); 
#endif


#if BFS_MAP==1
    // Mark all the vertices as not visited 
    for(int i = 0; i < _graph_vertices; i++)  _bfs_visited[i] = false; 
  ...
  for(int i=0; i<_graph_vertices; ++i) { // not sure if needed
  ...
  if(_config->_algo==gcn && _config->_hats==1 && _config->_inter_task_reorder==1) { // static reconfiguration
    int part_size = ceil(FEAT_LEN*FEAT_LEN/float((line_size/message_size)));
...
#if NETWORK==1
    if(_config->_perfect_lb==0 && num_banks>16) { // scale to num_banks
            ...
            for(auto it2=(*it).second.begin(); it2!=(*it).second.end();) {
              if(it2->dst_id==cur_tuple.dst_id) {
                if(cur_tuple.new_dist<it2->new_dist) {
                   ...
                   came_in=true;
                   it2 = (*it).second.erase(it2);
                   if((*it).second.empty()) { // if nothing left in list
...
// #if GRAPHMAT_SLICING==1 && REUSE>1 && WORKING_CACHE==0
  ...
  if(!l2hit) { // if(0)
    ...
    if(!sent_request) _mem_ctrl->_l2hits++;
...
void asic::execute_func(int src_core, int func, red_tuple cur_tuple) {
  ...
  switch(func) {
    case BANK_TO_XBAR: _bank_to_crossbar[(_bank_xbar_ptr+delay)%MAX_NET_LATENCY].push(cur_tuple);
                       ++_pending_updates;
      break;
    case ROUTER_XBAR_TO_BANK: 
      ...
      break;
    default: assert(0 && "Should not have come here");
      break;
  }
  _delayed_net_tasks++;
}
...
void asic::graphmat_slice_configs(red_tuple cur_tuple) {
  ...
  if(_finishing_phase || _switched_cache) {
    ...
    if(_finishing_phase) _stats->_stat_sync_finished_edges++;
    if(_switched_cache) _stats->_stat_extreme_finished_edges++;
    vertex_memory_access(cur_tuple);
  } else {
    vertex_memory_access(cur_tuple);
  ...
  if(_config->_cf) {
    for(int i=0; i<_graph_vertices; ++i) {
      for(int j=0; j<FEAT_LEN; ++j) {
        ...
        if(_config->is_sync()) {
          _scratch_vec[j][i] = 0;;
        } else { // for blocked async as well...
    ...
    if(_config->_algo==pr) {
      if(_config->_pull_mode) {
        ...
        for(int i=0; i<_graph_edges; ++i) {
          ...
          if(out_degree==0) continue; // cout << e.src_id << endl;
          assert(out_degree>0);
...
void asic::read_graph_structure(string file, int *offset, edge_info *neighbor, bool initialize_vd, bool is_csc_file) {
  ...
  while(fgets(linetoread, 5000, graph_file) != NULL) {
    ...
    if(is_csc_file || _graph_vertices==23947347 || _graph_vertices==6262104) {
      ...
      src = src-1;
      dst = dst-1;
    } else if(_graph_vertices==82000 || _graph_vertices==2700) { // for the movielens dataset (normal for pull mode)
      // char ignore;
      int extra;
      ...
      if(_graph_vertices==1632803) {
        ...
        if(_graph_vertices==44906 || _graph_vertices==1965206 || _graph_vertices==4847571 || _graph_vertices==2708 || _graph_vertices==1048576) {
        ...
        } else if(_graph_vertices==3774768 || _graph_vertices==2997166 || _graph_vertices==896308 || _graph_vertices==41652230 || _graph_vertices==23947347) { // cit-Patents, orkut, imdb, usa_road
          ...
          iss >> dst >> src;
          if(src==_graph_vertices) continue;
          dst = dst-1;
          src = src-1;
          wgt=1;
          if(!_config->_pr && !_config->_abfs) {
          ...
          if(_graph_vertices==7414866) { // matrix-market format
...
int asic::fill_correct_vertex_data() {
  ...
  while(!pq.empty()) {
    ...
    for(int i=offset[src_id]; i<offset[src_id+1]; ++i) {
      ...
      if(_scratch_ctrl->_is_visited[dst_id]) continue;
...
if(_config->_domain==true) return true;
#if CF==0
  for(int i=0; i<_graph_vertices; ++i) {
...
void asic::print_output() {
  ...
  for(int i=0; i<_graph_vertices; ++i) {
    ...
    if(_correct_vertex_data[i]!=_scratch[i]) {
      ...
      if(_correct_vertex_data[i]>10000) { continue; }
      error++;
      cout << "ERROR::";
...
void asic::initialize_simulation() {
  ...
  for(int i=0; i<core_cnt; ++i) lane_count[i]=0;

  if(!_config->is_slice()) { // sliced execution thing is done separately
    if(_config->is_non_frontier()) {
      ...
      for(int i=0; i<_graph_vertices; ++i) {
        ...
        if(i%10000==0) cout << "10000 vertices inserted" << endl;
        if(_offset[v+1]==_offset[v]) continue; // if outgoing degree==0
        // TODO: vidushi: only allowed for dangling vertices
        if(_config->_algo==pr && _vertex_data[v]==0) continue;
  ...
  _prev_commit_id=0; _prev_commit_core_id=0;
  if(_config->is_non_frontier()) _virtual_finished_counter=1;
}
...
void asic::update_stats_per_cycle() {
  ...
  for(int c=0; c<core_cnt; ++c) {
    ...
    if(_stats->_stat_enq_this_cycle[c]!=0) _tot_active_cycles[c]++;
...
#endif
   if(_switched_async && !_switched_cache) go_in=true;
   if(go_in) {
    ...
    if(cons==0) { // if slice_iter should be 0.25
...
int asic::task_recreation_during_sync(int slice_id, bool push_in_worklist, int reuse_factor) {
  ...
  for(int vid=slice_id*_scratch_ctrl->_slice_size; vid<(slice_id+1)*_scratch_ctrl->_slice_size && vid<_graph_vertices; ++vid) {
    ...
    switch(_config->_algo) {
      case gcn: spawn=_stats->_stat_barrier_count<GCN_LAYERS;
      ...
      case tc:
               spawn = false;
               break;
      case pr: spawn = should_spawn_task(_vertex_data[vid], _scratch[vid]);
               break;
      default: spawn = _scratch[vid] < _vertex_data[vid];
               break;
    };
...
int asic::task_recreation_during_sync() { // for non-sliced execution
    _stats->_stat_barrier_count++;
  ...
  for(int i=0; i<LADIES_SAMPLE; ++i) _correct_vertex_data[i]=0;
  ...
  for(int i=0; i<tot_vertices; ++i) {
    if(_config->is_non_frontier() && _config->_algo!=cf && _offset[i]==_offset[i+1]) continue;
    ...
    switch(_config->_algo) {
      case gcn: spawn=_stats->_stat_barrier_count<=GCN_LAYERS;
                // mult_spawn = _stats->_stat_barrier_count<=GCN_LAYERS;
                break;
      case cf:  spawn = should_cf_spawn_task(i);
                break;
        case pr: spawn = should_spawn_task(_vertex_data[i], _scratch[i]);
                 break;
        default: spawn = _scratch[i] < _vertex_data[i];
                 break;
    };
     ...
     _elem_dispatch=0;
      ...
      while(_elem_dispatch<max_elem_dispatch && count<core_cnt) {
        ...
        if(_config->_prac && (!_config->_fifo || _switched_async)) {
           ...
           if(max_serves==0) max_serves = _cur_cycle%2; // this is for 0.5
        ...
        for(int call=0; call<call_time && _elem_dispatch<max_elem_dispatch; ++call) {
          served_left=max_serves;
          for(int sample=0; sample<max_serves && served_left!=0; ++sample) { // TODO: that periodic access should be applied here
          ...
          if(!_asic_cores[_which_disp_ptr]->_task_queue[tqid][0].empty() || _asic_cores[_which_disp_ptr]->_pending_task[tqid].vid!=-1) {
            ...
            if(_elem_dispatch<max_elem_dispatch) {
              ...
              if(_config->_dramsim_enabled) {
                ...
                if(_config->is_sync()) { cb_deadlock_barrier *= 2; }
                // FIXME: checking: remove
      ...
      while(_elem_dispatch<x && count<core_cnt) { // to prevent infinite loop
...
void asic::simulate() {
  ...
  while(_cur_cycle<_max_iter) {
    ...
    /*for(int c=0; c<core_cnt; ++c) {
      ...
      for (int b = 0; b < core_cnt; ++b) {
        for (int a=0; a<_fine_alloc[b]; ++a) {
              ...
              if (_config->_net == crossbar) break;
          }
      }
      ...
      if (_config->_algo == gcn || _config->_domain==tree) {
        ...
        for (int i = _last_dispatch_core, j = 0; j < core_cnt; ++j, i = (i + 1) % core_cnt) {
          ...
          for (int a=0; a < _coarse_alloc[i]; ++a) {
            if (_mult_cores[i]->cycle(a)) {// continue if any core was working
                term_flag = false;
            }
      ...
      for (int i = 0; i < core_cnt; ++i) {
        for (int a=0; a < _fine_alloc[i]; ++a) {
          if(_asic_cores[i]->cycle()) {// continue if any core was working
    ...
    for (int i = 0; i < core_cnt; ++i) {
        if (!_asic_cores[i]->pipeline_inactive(false)) {// continue if any core was working
            term_flag = false;
        }
        if (!_mult_cores[i]->pipeline_inactive(false)) {// continue if any core was working
            term_flag = false;
        }
    }
    ...
    if(_config->_graphmat_slicing==0 && _config->_algo==gcn && term_flag && (_stats->_stat_barrier_count==(GCN_LAYERS-1)) && _config->_agg_mult_type==global) {
      ...
      _stats->_stat_barrier_count++;
      for(int c=0; c<core_cnt; ++c) {
        _fine_alloc[c]=1; _coarse_alloc[c]=1;
...
// SOMEHOW IT IS NOT COMING HERE FOR SLICE_COUNT=1 (oh maybe earlier)
   ...
   if(_cycles_per_iteration>=10) {
     ...
     if(_start_active_vertices<DYN_THRES) { // if new active vertices are more than 10x, then we should switch back
   ...
   if(_cycles_per_iteration>=1000) {
    if(1) { // }_mem_eff_last_iteration!=0 && mem_eff!=0) {
     ...
     if(mem_eff>128) { // if it improves beyond a certain point?
           _switched_cache=false;
           _cache_tipping_point=-1;
#if GRAPHMAT==1
         _switched_async=false;
   ...
   // this should be at the later iterations
        if(_cycles_per_iteration>=1000 && _cur_cycle>100000 && mem_eff<0.9*PRED_THR) { // PRED_THR for sp.lj.non_sliced is 18*message_size
   ...
   if(mem_eff>0.8*PRED_THR) { // if it improves beyond a certain point?
           _switched_cache=false;
           _cache_tipping_point=-1;
#if GRAPHMAT==1
         _switched_async=false;
...
void asic::calc_and_print_training_error() {
  ...
  for(int i=0; i<V; ++i) {
    ...
    if(_scratch[i]==MAX_TIMESTAMP) { // just because the initial random is too bad here..
      /*if(i==4) {
        cout << "diff: " << diff << endl;
      }*/
...
void asic::flush_multiplication_pipeline(int core_id) {
  ...
  while(!term_flag) {
    ...
    // TODO: we need to check at each core??
    if(!_mult_cores[core_id]->_process_reduce[1].empty()) term_flag=false;
    if(!_mult_cores[core_id]->_lsq_process[1].empty()) term_flag=false;
    if(!_mult_cores[core_id]->_scratch_process[1].empty()) term_flag=false;
    // if(_mult_cores[core_id]->_pending_task.weight_rows_done==0) term_flag=true;
    if(!_coarse_reorder_buf[core_id][1]->pipeline_inactive(false)) term_flag=false;
    ++cycles_taken;
  }
...
void asic::update_heterogeneous_core_throughput() {
  ...
  if(_config->_core_cnt==1) { // ideal scenario
 ...
 for(int c=0; c<core_cnt; ++c) _fine_inactive[c]=true;
 ...
 for(int c=0; c<core_cnt; c+=2) {
    if(cores_to_agg>0 || c<2) {
      if(c>1) flush_multiplication_pipeline(c);
      _asic_cores[c]->_fine_grain_throughput=1*_config->_process_thr;
      --cores_to_agg;
...
void asic::print_local_iteration_stats(int slice_id, int reuse) {
  if(_config->_dyn_graph==1 && _config->_reuse_stats==0) { // TODO: turning off for now
    return;
  }
  ...
  for(int i=0; i<end; ++i) {
    ...
    sum += cur_updates;
    if(cur_updates>max) max=cur_updates;
  }
  // cout << endl;
...
void asic::print_stats() {
  if(_already_printed_stats) return;
  ...
  int dangling=0;
  for(int i=0; i<LADIES_SAMPLE; ++i) {
    if(_is_dest_visited.test(i)==0) dangling++;
  }
  ...
  if(_config->_tc) {
    for(int i=0, end=MAX_DFGS; i<end; ++i) {
      cout << _gcn_updates[i] << " ";
      sum += _gcn_updates[i];
      if(_gcn_updates[i]>max) max=_gcn_updates[i];
    }
  } else {
    for(int i=0, end=core_cnt*num_banks; i<end; ++i) {
      cout << _gcn_updates[i] << " ";
...
DTYPE asic::process(DTYPE edge_wgt, DTYPE dist) { // (edge_wgt, dist)
  if(_config->_algo==cf) {
...
void asic::simulate_ideal() {
  ...
  for(int i=0; i<_graph_vertices; ++i) _map_vertex_to_core[i]=-1;
  for(int i=0; i<core_cnt; ++i) _stats->_stat_dist_tasks_dequed[i]=0;
  for(int i=0; i<core_cnt; ++i) _stats->_stat_dist_edges_dequed[i]=0;
  ...
  while(_cur_cycle<_max_iter) {
    ...
    for(int c=0; c<core_cnt; ++c) {
      ...
      if(!_asic_cores[c]->pipeline_inactive(false)) term_flag=false;
  ...
  if(_cur_cycle==_max_iter) {
    cout << "Didn't complete; vertices mapped: " << tot_vertices_mapped <<
    " total vertices done: " << _stats->_stat_vertices_done << "\n";
    return;
  }
  else cout << "Program done in cycles: " << _cur_cycle << endl;

  int degree_per_core[core_cnt];
  for(int i=0; i<core_cnt; ++i) degree_per_core[i]=0;
  ...
  for(int i=0; i<_graph_vertices; ++i) {
    if(_map_vertex_to_core[i]==-1) continue;
    cur_max_freq=0;
    expected_core=-1;
    for(int c=0; c<core_cnt; ++c) {
  ...
  for(int i=0; i<core_cnt; ++i) {
    ...
    if(degree_per_core[i]>max) max = degree_per_core[i];
    degree_per_core[i]=0;
  }
...
void asic::test_mult_task() {
  ...
  while(_cur_cycle<max_iter) {
    ...
    for(int c=0; c<_config->_core_cnt; ++c) {
      if(_mult_cores[c]->cycle(0)){ // continue if any core was working
        term_flag=false;
      }
    }
...
int main() {
  ...
  if(model->_config->_hash_join==1) {
    ...
    while(model->_cur_cycle<model->_max_iter) {
      ...
      pref_tuple cur_tuple;
      if(model->_cur_cycle<10000) { // 1600k $-lines out of 16k => Reuse=100 times
    ...
    // #endif
      ...
      if(hits_this_cycle==0 && model->_cycles_this_phase>100 && (model->_unique_mem_reqs_this_phase>(0.9*unique_cache_lines_per_slice))) { // this is the reuse distance..
      ...
      term_flag=true;
      for(int c=0; c<core_cnt; ++c) {
        if(!model->_asic_cores[c]->pipeline_inactive(false)) term_flag=false;
      }
      if(!model->_hit_addresses.empty()) term_flag=false;
      if(!model->_miss_addresses.empty()) term_flag=false;
      ...
      if(model->_cur_cycle<10000) term_flag=false;
      if(term_flag) break;
      ++model->_cur_cycle;
    }
  ...
  if(_graph_vertices==2394385) { // wiki
    start_vert=2;
  }
...
void asic::synchronizing_sync_reuse_lost_updates(int reuse_factor) {
  ...
  for(int reuse=0; reuse<reuse_factor-1; ++reuse) {
    ...
    for(unsigned j=0; j<_delta_updates[reuse].size(); ++j) {
      ...
      if(_config->_algo==gcn) {
        if(src_id==prev_vid) continue; // sure it will continue from innermost loop?
        prev_vid=src_id;
        ...
        if(should_spawn) _scratch[src_id] = _vertex_data[src_id];
      }
    ...
    for(int left=0; left<reuse_factor-reuse-1; ++left) {
      ...
      for(int i=0; i<_graph_vertices; ++i) {
        ...
        if(spawn) {
          task_entry new_task(i,_offset[i]);
          _stats->_tot_activated_vertices++;
          if(left==(reuse_factor-reuse-2)) { // last tasks should be in memory
...
#elif CF==1
          if(should_cf_spawn_task(i)) { // copy should_spawn_task logic here
...
# if ABCD==1
    ...
    for(int reuse=0; reuse<reuse_factor-1; ++reuse) {
      ...
      for(unsigned j=0; j<_delta_updates[reuse].size(); ++j) {
        ...
        if(reuse<reuse_factor-1) { // need to apply all tasks always
#elif CF==1
        // FIXME: THIS IS WRONG!!!
        // int old_dist = _vertex_data[src_id];
        // _vertex_data[src_id] = reduce(_vertex_data[src_id], _delta_updates[reuse][j].second);
      ...
      cout << "Number of tasks pushed after compute phase: " << num_tasks_pushed << endl;
      // FIXME: CHECK!!
      while(_delta_updates[reuse_factor-1].size()>0) _delta_updates[reuse_factor-1].pop_back();
...
void asic::simulate_slice() {
  ...
  while(!_task_ctrl->worklists_empty() || _task_ctrl->tot_pending_tasks()>0) {
    ...
    if(outside) print_local_iteration_stats(0,0);
    ...
    while(served<_slice_count && _global_iteration<SLICE_NUM_ITER) {
      ...
      if(_config->_abort || _config->_entry_abort) {
        ...
        for(int a=0; a<core_cnt; ++a) { _task_ctrl->_present_in_local_queue[a].reset(); }
      }
...
#if PR==1 && CF==0
        if(cur_task.dist!=0) { // don't push if residual=0, meaning no incoming value
#else
          if(1) {
#endif
#if WORK_STEALING==0
...
void asic::ladies_sampling() {
  ...
  while(fgets(linetoread, 50000, adj_new) != NULL) {
    ...
    if(row%2==0) cout << "Total edges in layer l: " << l << " is: " << _offset[LADIES_SAMPLE] << endl;
    ++row;
  }
  fclose(adj_new);
  ...
  for(i=0; i<_graph_vertices; ++i) { // all vertices
    int out_degree = _offset[i+1]-_offset[i];
    for(j=_offset[i]; j<_offset[i+1]; ++j) { // index of the corresponding edge
  ...
  for(int l=GCN_LAYERS-1; l>=0; --l) {
    ...
    for(i=0; i<_graph_vertices; ++i) prob[i]=0;
...
void asic::scalability_test(int scale, int delay) {
  ...
  if(it==_asic_cores[0]->_task_queue[0][0].end()) {
      list<task_entry> a; a.push_back(new_task);
  ...
  while(!_asic_cores[0]->_task_queue[0][0].empty() || pending_tasks>0) {
      ...
      while(num_edges<scale && !_asic_cores[0]->_task_queue[0][0].empty()) {
        ...
        while(!new_tasks[cur_ptr].empty()) {
            ...
            if(_config->_task_sched==abcd) { prio = new_task.delta_prio; }
            auto it = _asic_cores[0]->_task_queue[0][0].find(prio);
            if(it==_asic_cores[0]->_task_queue[0][0].end()) {
                list<task_entry> a; a.push_back(new_task);
                _asic_cores[0]->_task_queue[0][0].insert(make_pair(prio,a));
    ...
    for(int i=0; i<_graph_vertices; ++i) _vertex_data[i]=_scratch[i];
    correctness_check();
}
...
int task_controller::get_cache_hit_aware_priority(int core_id, bool copy_already_present, int pointer_into_batch, int leaf_id, int batch_width, int num_in_pointer) {
  ...
  if(pointer_into_batch>=0) {
    if(_asic->_config->_algo==gcn) return 0; 
  ...
  if(_asic->_config->_algo==gcn) {
   ...
   if(priority==0) { // ie. cache hit (mostly are misses so can't do anything?)
   ...
   if(_asic->_config->_algo!=gcn) {
     ...
     if(full_batch_factor<1) priority/=2; // if full, prioritize these over hits
...
void task_controller::central_task_schedule() {
 ...
 if(_asic->_cur_cycle%500==0 && _asic->_cur_cycle!=0) {
   ...
   for(int i=0; i<core_cnt; ++i) { // TODO: logically should add extra dimension of number of task queues
     for(int j=0; j<NUM_TQ_PER_CORE; ++j) {
       if(_asic->_asic_cores[i]->_task_queue[j][0].size()<10) continue;
       auto it = _asic->_asic_cores[i]->_task_queue[j][0].rbegin();
       if(it!=_asic->_asic_cores[i]->_task_queue[j][0].rend()) { // should have reasonable gap (may increase cycle range?)
         count++;
         // _avg_timestamp += it->first;
         if(it->first<min_timestamp) min_timestamp=it->first;
         // auto it2 = _asic_cores[i]->_task_queue[0].begin();
  ...
  // _avg_timestamp = 1.02*max_timestamp;
   ...
   else _avg_timestamp = MAX_TIMESTAMP;
...
void task_controller::insert_global_task(DTYPE timestamp, task_entry cur_task) {
  ...
  for(auto it2=_global_task_queue.begin(); it2!=_global_task_queue.end();) {
    ...
    for(auto it3=(*it2).second.begin(); it3!=(*it2).second.end();) {
      if((*it3).vid==cur_task.vid) {
        ...
        if((*it2).second.empty()) { // if nothing left in list
          came_inside = true;
...
#if ENTRY_ABORT==1
  if(_present_in_queue.test(cur_task.vid)==1) return;
   ...
   */
   ...
   if(it==_global_task_queue.end()) {
     ...
     list<task_entry> a; a.push_back(cur_task);
...
DTYPE task_controller::calc_cache_hit_priority(int vid) {
  ...
  if (!l2hit) { // FIXME: How is this only 16?
    priority = 10;
...
*/
  ...
  if(_asic->_config->_pull_mode) {
    ...
    for(int e=_asic->_csr_offset[cur_task.vid]; e<_asic->_csr_offset[cur_task.vid+1]; ++e) {
      ...
      if(_present_in_local_queue[allotted_core].test(vid)==0) { // barrier tasks are also considered online here
...
void asic::insert_global_bank(int bank_id, DTYPE priority, red_tuple cur_tuple) {
  ...
  if(it==_bank_queues[bank_id].end()) {
    list<red_tuple> a; a.push_back(cur_tuple); // front(cur_task);
...
// #if SGU == 1 && PRAC==0 && FIFO==0 && ABORT==1// delete any other task with same vid
  ...
  abort=true;
#endif
  if(_asic->_switched_async) abort=true;
  if(abort) {
    // want to scan whole whole task queue
...
// #if ABORT==1
  ...
  if(it==_asic->_asic_cores[core_id]->_task_queue[tqid][lane_id].end()) {
    list<task_entry> a; a.push_back(cur_task); // front(cur_task);
  ...
  for(int i=0; i<core_cnt; ++i) {
    if(_asic->_asic_cores[i]->_task_queue[0][0].empty()) {
      ...
      while(_asic->_asic_cores[random_core]->_task_queue[0][0].size()>10 && elem_done<5) {
        ...
        if(it->first>_avg_timestamp) break;
      ...
      // if(_asic_cores[random_core]->_task_queue[0].size()>1) {
        ...
        while(!(it->second).empty()) { // Let's try this for better load balancing (extract all values from later timestamp)
          task_entry cur_task = (it->second).front();
    ...
    for(int tqid=0; tqid<NUM_TQ_PER_CORE; ++tqid) {
      for(auto it=_asic->_asic_cores[a]->_task_queue[tqid][0].begin(); it!=_asic->_asic_cores[a]->_task_queue[tqid][0].end();) {
        ...
        for(auto it3=(*it).second.begin(); it3!=(*it).second.end();) {
          if((*it3).start_offset!=_asic->_offset[(*it3).vid]) { // don't want to pop
#if GRAPHMAT_SLICING==1 || BLOCKED_ASYNC==1
...
bool task_controller::can_push_coarse_grain_task() {
  ...
  if(are_two_tasks_ready) return true;
  if(!are_two_tasks_ready && !_coarse_task_queue.empty()) {
    if(_asic->_cur_cycle-_coarse_task_queue.front().entry_cycle>MAX_DELAY) return true;
  }
  return false;
}
...
void task_controller::distribute_lb() {
  ...
  if(_global_task_queue.empty()) return;
  int num_pops=0;
  for(auto it=_global_task_queue.begin(); !_global_task_queue.empty() && num_pops<256;) { // no limit on pop count to emulate decentralized system
   ...
   if(_asic->_config->_entry_abort) {
    ...
    if((it->second).empty()) {
      _global_task_queue.erase(it->first);
      // ++it;
    } else { // ++it; // we need to work with the same it->second queue
    }
    it=_global_task_queue.begin();
    num_pops++;
  }
...
void task_controller::distribute_prio() {
  ...
  for(auto it=_global_task_queue.begin(); !_global_task_queue.empty();) { // no limit on pop count to emulate decentralized system
    assert(!_global_task_queue.empty());
...
void task_controller::distribute_one_task_per_core() {
  ...
  // cout << "Global task queue size: " << _global_task_queue.size() << endl;
  int num_pops=0;
  for(auto it=_global_task_queue.begin(); !_global_task_queue.empty() && num_pops<core_cnt;) { // no limit on pop count to emulate decentralized system
    assert(!_global_task_queue.empty());
    task_entry cur_task = (it->second).front(); // _global_task_queue[t].front();
...
bool task_controller::worklists_empty() {
  for(int i=0; i<_asic->_slice_count; ++i) {
    ...
    if(_worklists[i].size()>0) return false;
#else
    if(!_worklists[i].empty()) return false;
#endif
  }
...
void task_controller::distribute2() {
  ...
  for(auto it=_global_task_queue.begin(); !_global_task_queue.empty() && it!=_global_task_queue.end() && num_edges_processed<process_thr;) { //_max_elem_mem_load;) { 
    assert(!_global_task_queue.empty());
    assert(!(it->second).empty());
    task_entry cur_task = (it->second).front();
...
// this is causing 5x drop in performance even with ideal net?
    ...
    for(int i=0; i<2; ++i) { // 4*4 byes (edge weight, dist)
      assigned_core = _asic->_scratch_ctrl->get_local_scratch_id(cur_task.vid);
...
void memory_controller::read_complete(unsigned data, uint64_t address, uint64_t clock_cycle) {
  ...
  if(_asic->_finishing_phase) _asic->_stats->_stat_dram_add_on_access++;
  ...
  for(auto req=_outstanding_mem_req[address].begin(); req!=_outstanding_mem_req[address].end();) {
    ...
    if((_asic->_config->_graphmat_slicing) && (_asic->_finishing_phase || _asic->_switched_cache)) go_in=true;
    if(go_in) {
      int assoc_core=req_core_id;
      if(cb_buf_entry==-1) {
        _num_pending_cache_reqs--;
        if(return_to_lsq.update_width>0) { // wide gcn packet
          ...
          red_tuple cur_tuple;
          cur_tuple.dst_id = dst_id; cur_tuple.new_dist = return_to_lsq.src_dist;
...
void scratch_controller::receive_scratch_request() { // TODO: should call this first??
  ...
  while(!_net_data.empty()) {
    ...
    if(remote_read_data.packet_type==scalarResponse && remote_read_data.new_dist==-1) { // NOT AN IDEAL WAY
...
void memory_controller::update_cache_on_new_access(int core_id, int edge_id, int cur_prio_info) {
  ...
  for(int a=0; a<assoc; ++a) {
    ...
    if(cur_prio<=min_prio) { // for same prio, always replace 0
      min_prio=cur_prio;
      entry_id = a;
    }
  }
...
bool memory_controller::is_cache_hit(int core, Addr_t paddr, UINT32 accessType, bool updateReplacement, UINT32 privateBankID) {
  ...
  switch(_asic->_config->_cache_repl) {
    case phi: return _l2_cache->LookupAndFillCache(paddr, ACCESS_LOAD, true, 0);
              break;
    case allhits: return true;
                  break;
    case allmiss: return false;
                  break;
    default: break;
  }
...
void scratch_controller::optimize_load() {
  if(_asic->_config->_noload) return;
  ...
  // if(1) {
    ...
    while(cur_v < _asic->_graph_vertices) _num_spec[++cur_v] = cur_e+1;
    ...
    for(int i=1; i<=_asic->_graph_vertices; ++i) {
      ...
      if(_asic->_config->_algo!=cf) { _asic->_vertex_data[i-1] = _asic->_scratch[i-1]; }
    ...
    for(int i=0; i<_asic->_graph_vertices; ++i) _mapping[i]=i;
  ...
  for(int slice_num=0; slice_num<count; ++slice_num) {
    for(int s=0; s<_slice_size; ++s) {
      for(int d=_asic->_offset[s]; d<_asic->_offset[s+1]; ++d) {
        ...
        if(dst_slice!=slice_num) continue;
...
void scratch_controller::locality_test(int slice_id) {
  ...
  if(remote_count==0 && core_cnt!=1) assert(0 && "mapping is incorrect");
...
scratch_controller::scratch_controller(asic* host, stats* stat) : _asic(host) {
  ...
  for(int j=0; j<_asic->_graph_vertices; ++j) _in_degree[j]=0;
#endif
  ...
  for (int i = 0; i < _asic->_graph_vertices; i++) _visited[i] = false;
#if GRAPHMAT==1 || BLOCKED_ASYNC==1
...
void scratch_controller::map_grp_to_core() {
  ...
  for(int central=0; central<4; ++central) {
    for(int round=0; round<3; ++round) {
      for(int i=0; i<core_cnt; ++i) {
        if(_map_grp_to_core[i]!=-1) continue;
  ...
  // cout << "Starting dfs with start vertex s: " << s << " and slice id: " << slice_id << " and size: " << _slice_size << " and start vertices visited: " << _vertices_visited << endl; 
    ...
    { 
        ...
        { 
          ...
          if(_vertices_visited>MINIBATCH_SIZE) return;
#endif
            _mapping[s] = ++_global_ind;
        ...
        for (int i = _asic->_offset[s]; i < _asic->_offset[s+1]; ++i) {
          ...
          if(dest_slice!=slice_id) continue;
          if (!_visited[_asic->_neighbor[i].dst_id]) {
            // cout << _neighbor[i].dst_id << " ";
...
{ 
    ...
    { 
        ...
        for (i = _asic->_offset[s]; i < _asic->_offset[s+1]; ++i) { 
          if(slice_id!=get_slice_num(_asic->_neighbor[i].dst_id)) continue;
...
int scratch_controller::get_mc_core_id(int dst_id) {
  ...
  switch(mc_id) {
    case 0: core_id=rand_num; 
      break;
    case 1: core_id=rand_num*num_rows+num_rows-1;
      break;
    case 2: core_id=rand_num+num_rows*(num_rows-1);
      break;
    case 3: core_id=rand_num*num_rows;
      break;
    default: cout << "Wrong mc id"; exit(0);
      break;
  }
...
int scratch_controller::get_global_bank_id(int dst_id) {
  ...
  if(_asic->_config->_prac==0) { // Why do?
    ...
    if(dst_id<256) { // should be different for each source access
  ...
  if(_asic->_config->_exec_model==async_slicing || _asic->_config->_exec_model==sync_slicing || _asic->_config->_exec_model==blocked_async) {
    for(int i=0; i<_asic->_slice_count; ++i) {
      switch(_asic->_config->_spatial_part) {
        case bdfs: perform_bdfs_load_balancing(i);
                   break;
        case linear: perform_linear_load_balancing(i);
                   break;
        case modulo: perform_modulo_load_balancing(i);
                   break;
        case random_spatial: 
             ...
             break;
        default: perform_load_balancing2(i);
                 break;
      }
    }
  }
...
void scratch_controller::generate_metis_format() {
  ...
  if(e%2==0) e=E/2; //very aggressively assuming undirected graph
  ...
  metis_input.close();
  if(e!=0) cout << "Extra edges than marked: " << e << endl;
  exit(0);
  // call metis function
...
void scratch_controller::read_mongoose_slicing() {
  ...
  char linetoread[5000];
  for(int i=0; i<_asic->_slice_count; ++i) _asic->_slice_size_exact[i]=-1;
  while(fgets(linetoread, 5000, graph_file) != NULL) {
    std::string raw(linetoread);
    std::istringstream iss(raw.c_str());
    int a,b;
...
void scratch_controller::read_metis_slicing() {
  ...
  for(int i=0; i<_asic->_metis_slice_count; ++i) _asic->_slice_size_exact[i]=-1;
  int a=-1;
  int b;
#if RANDOM==1
  for(int i=0; i<_asic->_graph_vertices; ++i) {
...
void scratch_controller::get_sync_boundary_nodes() {
  ...
  for(int i=0; i<_asic->_graph_vertices; ++i) _num_slices_it_is_copy_vertex[i]=0;

  cout << "Came in to get slice boundary nodes\n";
  // read from mongoose to get slice number..
  int boundary_nodes[_asic->_slice_count];
  for(int i=0; i<core_cnt; ++i) _asic->_task_ctrl->_present_in_local_queue[i].reset();
...
void scratch_controller::get_linear_sgu_boundary_vertices() {
  ...
  for(int i=0; i<_asic->_slice_count; ++i) { // this should be in order of dfs
    _node_done.reset();

    int v = i*_slice_size;
    for(int a=0; a<_slice_size; ++a) { // for all vertices in this slice
      v += 1;
      for(int e=_asic->_offset[v]; e<_asic->_offset[v+1]; ++e) { // for all edges in this slice
...
void scratch_controller::get_slice_boundary_nodes() {
 ...
 for(int k=0; k<_slice_count; ++k) { 
  ...
  // something)
#if GCN==0 && CF==0
  for(int i=0; i<_asic->_graph_vertices; ++i) _asic->_vertex_data[i]=_asic->_correct_vertex_data[i];
#endif
  int cur_e=-1;
  int cur_v=0;
  for(int i=0; i<_asic->_slice_count; ++i) {
  ...
  for(int i=0; i<_asic->_slice_count; ++i) free(_slice_vids[i]);
  ...
  while(!pq.empty()) {
    ...
    for(int i=_asic->_offset[src_id]; i<_asic->_offset[src_id+1]; ++i) {
      int dst_id = _asic->_neighbor[i].dst_id;
      if(_is_visited[dst_id]) continue;
  ...
  for(int i=0; i<_asic->_graph_vertices; ++i) _asic->_scratch[i]=MAX_TIMESTAMP-1;
...
void scratch_controller::graphicionado_slice_fill() {
    ...
    for(int i=0; i<_asic->_graph_vertices; ++i) {
        ...
        for(int e=_asic->_offset[i]; e<_asic->_offset[i+1]; ++e) {
            ...
            if(src_slice!=dest_slice) ++worst_case_extra_traffic;
