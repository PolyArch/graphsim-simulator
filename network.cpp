...
void network::fill_routing_table() {
  ...
  for(int i=0; i<core_cnt; ++i) {
    for(int j=0; j<core_cnt; ++j) {
      ...
      if((cur_X+1)%num_rows!=0) { // not the boundary cases
        if(X!=cur_X) {
...
int network::calc_hops(int src, int dest) {
  ...
  // sum
  assert(x_disp>=0);  assert(y_disp>=0);
...
int network::get_link_dest_id(int int_link_id) {
  ...
  int num_horizontal = num_rows*(num_rows-1);
  if(int_link_id<num_horizontal) horizontal_link=true;
...
void network::update_pops_acc_to_bandwidth(net_packet cur_tuple, int &num_pops) {
  if(!is_remote_read(cur_tuple)) { // simple req
...
void network::decode_real_multicast(net_packet cur_tuple, int core_id) {
  for(int k=0; k<cur_tuple.multicast_dst_core_id.size(); ++k) {
    ...
    } else { // for other case when we need to push to bank queues
...
void network::decode_path_multicast(net_packet cur_tuple, int core_id) {
  ...
  for(int k=0; k<cur_tuple.multicast_dst_core_id.size(); ++k) {
    ...
    } else { // create entry into the remote tuple, while finding the min_dist
...
void network::encode_real_multicast(net_packet cur_tuple, int core_id) {
  ...
  // run over all states
  for(int s=0; s<3; ++s) {
    if(sorted_dest[s].size()==0) continue;
...
pref_tuple network::convert_net_to_pref(net_packet packet, int k) {
  ...
  if(_asic->_config->_cf==1 && _asic->_config->_algo==gcn) {
    for(int f=0; f<FEAT_LEN; ++f) return_to_arob.vertex_data[f] = packet.vertex_data[f];
  }
...
net_packet network::create_scalar_request_packet(int src_id, int dst_id, int req_core, int cb_entry, bool second_buffer=false) {
  ...
  packet.src_id = src_id; packet.dst_id = dst_id;
  packet.second_buffer = second_buffer;
  packet.req_core_id = req_core; packet.cb_entry = cb_entry;
  packet.dst_id = dst_id;
...
net_packet network::create_noopt_scalar_update_packet(int src_id, int dst_id, DTYPE new_dist) {
  ...
  packet.src_id = src_id; packet.dst_id = dst_id;
  packet.req_core_id = -1; packet.cb_entry = -1;
  packet.new_dist = new_dist;
  packet.dst_id = dst_id; packet.dest_core_id = _asic->_scratch_ctrl->get_local_scratch_id(dst_id);
  // packet.tid = tid;
  return packet;
}
...
net_packet network::create_scalar_update_packet(int tid, int src_id, vector<iPair> multicast_dst_wgt, DTYPE new_dist) {
  net_packet packet;
  packet.packet_type = scalarUpdate;
  packet.src_id = -1; packet.dst_id = -1;
  packet.req_core_id = -1; packet.cb_entry = -1;
  packet.new_dist = new_dist;
  packet.dst_id = -1; packet.dest_core_id = -1;
...
void network::pull_packets_to_network(int core_id) {
  ...
  while(!_net_in_buffer[core_id].empty() && num_pops<process_thr) {
    ...
    if(dest_core_id==core_id) { // no need to push to network
      _stat_local_updates++;
      ...
      if(dest_core_id!=-1) _stat_packets_to_router[dest_core_id]++;
...
void network::push_packets_from_network(int core_id) {
  ...
  while(!_net_out_buffer[core_id].empty() && num_pops<process_thr) {
    ...
    if(cur_tuple.req_core_id!=-1) {
      ...
      if(cur_tuple.packet_type!=dummy) { // there should be my output
...
void network::transfer_scalar_packet(net_packet cur_tuple, int cur_core_id, int i, int j, int &num_pops) {
  ...
  assert(next_dest!=-1);
  if(get_link_dest_id(i)==next_dest) { // moving over the link
    // assert(j<2);
    if(_bus_available[i]) { // good to go (cur_link is available)
  ...
  } else { // need to switch links in same router, find new i and j
    ...
    if(next_dest==cur_core_id+1) {
      ...
      dest1=i+num_rows; dest2=0;
    } else if(next_dest==cur_core_id-num_rows) {
      dest1=i-num_rows; dest2=3;
    } else {
...
void network::transfer_multicast_packet(net_packet cur_tuple, int cur_core_id, int i, int j, int &num_pops) {
  ...
  for(int d=0; d<cur_tuple.multicast_dst_core_id.size(); ++d) {
    ...
    if(cur_dst_core==cur_core_id) state=0;
    else if(next_dest==get_link_dest_id(i)) state=1;
    else if(next_dest==cur_core_id+1) state=2;
    else if (next_dest==cur_core_id-1) state=3;
    else if (next_dest==cur_core_id+num_rows) state=4;
    else if (next_dest==cur_core_id-num_rows) state=5;
    else cout << "Invalid option in internal core\n";
  ...
  for(int s=0; s<6; ++s) {
    ...
    dest1=-1; dest2=-1;
    if(s==0) { // copy to the same core
       _net_out_buffer[sorted_dest[s][0]].push(cur_tuple);
    } else if(s==1) {
      if(_bus_available[s]) { // good to go (cur_link is available)
        // num_pops++;
        // _stat_tot_bw_utilization+=4;
...
void network::internal_routing() {
  ...
  for(int i=0; i<_num_links; ++i) {
    for(int j=0; j<4; ++j) {
      if(_internal_buffers[i][j].empty()) continue;
      int num_pops=0;
      while(!_internal_buffers[i][j].empty() && num_pops<_bus_vec_width) {
        ...
        if(_is_done[cur_tuple.label]) break; 
        ...
        if(j==0 || j==3) {
          ...
          int num_horizontal = num_rows*(num_rows-1);
          if(i<num_horizontal) { // if horizontal link
            cur_core_id = get_core_id(i)+1;
          } else { // if vertical link
            cur_core_id = get_core_id(i)+num_rows;
          }
        }

        if(dest_core_id==cur_core_id) { // reached destination
  ...
  for(int i=0; i<NUM_MC; ++i) {
    ...
    while(!_mc_buffers[i].empty() && cur_pops<process_thr) {
      ...
      if(_asic->_config->_hats==1) {
        ...
        if(1) { // _asic->_mult_cores[cur_tuple.dest_core_id]->_batch_free_list.size()>0) {
...
bool network::buffers_not_empty(bool show) {
  for(int i=0; i<NUM_MC; ++i) {
    if(!_mc_buffers[i].empty()) {
      if(show) cout << "memory controllers not empty\n";
      return true;
    }
  }
  for(int c=0; c<core_cnt; ++c) {
    if(!_net_in_buffer[c].empty()) {
      if(show) cout << "network in buffers not empty\n";
      return true;
    }
    if(!_net_out_buffer[c].empty()) {
      if(show) cout << "network out buffers not empty\n";
      return true;
    }
  }
  for(int i=0; i<_num_links; ++i) {
    for(int j=0; j<4; ++j) {
      if(!_internal_buffers[i][j].empty()) {
        if(show) cout << "internal buffers not empty\n";
        return true;
      }
    }
  }
...
bool network::cycle() {
  ...
  for(int c=0; c<core_cnt; ++c) {
    ...
    if(is_active) break;
    if(_asic->_config->_net==crossbar) break;
  }
  return is_active;
}
