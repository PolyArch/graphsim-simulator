...
enum slice_sched_type
{
  roundrobin,
  locality,
  priority
};
enum update_visibility
{
  synch,
  asynccoarse,
  asyncfine
...
enum work_dist_type
{
  addr_map,
  work_steal
}; // will having them mutually exclusive fix them? , perfect_lb};
enum cache_repl_type
{
  ...
  allhits,
  allmiss
}; // TODO: assuming only a single cache, not copies?
